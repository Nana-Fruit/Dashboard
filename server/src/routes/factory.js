...
factory.get("/summary", async (req, res, next) => {
  try {
    const records = inRange(loadRecords(), req.query);
    const rates = await store.getLaborRates();
...
factory.get("/rooms/:room", async (req, res, next) => {
  try {
...
factory.put("/labor-rates", requireEdit, async (req, res, next) => {
  try {
    const rates = req.body || {};
    const clean = {};
    for (const k of ROOM_ORDER) if (rates[k] != null) clean[k] = Number(rates[k]) || 0;
    res.json({ laborRatePerHour: await store.setLaborRates(clean) });
  } catch (err) {
    next(err);
  }
});
